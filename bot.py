...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_message = await update.message.reply_text("⚡ Extracting securely with Cookies... Please wait.")

    # Using the industry standard cookiefile method to completely bypass IP blocks
    ydl_opts = {
        'format': 'best[ext=mp4]/best', 
        'outtmpl': f'{DOWNLOAD_DIR}/%(id)s.%(ext)s',
        'quiet': True,
        'no_warnings': True,
        'cookiefile': './assets/cookies.txt', # The magic key that authenticates the request
...
def main():
    ...
    print("✅ Bot is running properly with Browser Cookies!")
    app.run_polling()
